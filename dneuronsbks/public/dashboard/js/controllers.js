function taxRuleCtrl($scope, taxRuleFactory ) {
	taxRuleFactory.getAllRules().success(function(data) {
		$scope.taxRuleList = data.resultdata;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});
}

function fileCtrl($scope, fileFactory ) {
	$scope.fileclassvalue ='';
	fileFactory.getAllFiles().success(function(data) {
		$scope.fileVoList = data.resultdata;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});
	$scope.changefileclass = function(fileclass) {
		$scope.fileclassvalue = fileclass;
	};
}

function reportFileCtrl($scope, fileFactory ) {
	fileFactory.getReportFiles().success(function(data) {
		$scope.fileVoList = data.resultdata;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});
...
function searchClientCtrl($scope,$http, $window, searchClientFactory)
{
	//Vendor suggestion
	  $scope.getSuggestion = function(val) {
	    return $http.post('/manageuser/suggestUser', {
	      params: {
	    	search: val,
	        sensor: false
	      }
	    }).then(function(response){
	      return response.data.resultdata.map(function(item){
	        return item;
	      });
	    });
	  };
	  ...
	  {
		  $scope.$item = $item;
		  var formData = {"clientId" : $item};
		  searchClientFactory.setClientId(formData).success(function(data) {
				$scope.clientSelected = "success";
...
function lightBoxCtrl($scope, $modal) {

	$scope.openVendorAdd = function () {

        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/vendor/add.html',
            controller: vendorAddCtrl,
            size: 'lg'
        });
    };
    
    $scope.openVendorEdit = function (vendorDisplayName) {
    	console.log("vendorDisplayName : " + vendorDisplayName);
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/vendor/edit.html',
    ...
    $scope.openCustomerAdd = function () {

        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/customer/add.html',
            controller: customerAddCtrl,
            size: 'lg'
        });
    };
    
    $scope.openCustomerEdit = function (customerDisplayName) {
    	console.log("customerDisplayName : " + customerDisplayName);
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/customer/edit.html',
    ...
    $scope.openBankAdd = function () {


        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/bank/add.html',
            controller: bankAddCtrl,
            size: 'lg'
        });
    };
    
    $scope.openBankEdit = function (bankDisplayName) {
    	console.log("bankDisplayName : " + bankDisplayName);
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/bank/edit.html',
    ...
    $scope.openEmployeeAdd = function () {

        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/employee/add.html',
    ...
    $scope.open2 = function () {
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/modal_example2.html',
    ...
    $scope.open3 = function (size) {
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/modal_example3.html',
            size: size,
            controller: ModalInstanceCtrl,
        });
    };
...
function vendorAddCtrl ($scope, $modalInstance, vendorFactory, accountingCodeFactory , taxRuleFactory, $filter ) {
	
	$scope.vendorForm = {};
	$scope.vendorForm.required=false;	
	$scope.vendorForm.showSuccessAlert = false;
	$scope.vendorForm.showErrorAlert = false;
	$scope.vendorForm.alerts1 = [
	                { type: 'danger', msg: 'aa' },
	                { type: 'success', msg: 'bb' },
	                { type: 'info', msg: 'cc' }]

	vendorFactory.getInitData().success(function(data) {
		$scope.codeMasterVo = data.resultdata;
		//$('#codeVo_chosen_select_chosen').trigger('chosen:updated');
		//console.log($scope.codeMasterVo);
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});	
	 ...
	 $scope.changeCountry = function(){
			...
			var country = $filter('filter')($scope.countries, {
				code : $scope.vendorForm.countryCode
			});	
			
			$scope.states = country[0].states;
    ...
    $scope.ok = function () {
    	...
    	vendorFactory.addVendor($scope.vendorForm).success(function(data) {
    		$scope.codeMasterVo = data.resultdata;
    		//console.log($scope.codeMasterVo);
    	}).error(function(data, status, headers, config) {
    		alert( "failure message: " + JSON.stringify({data: data}));
...
function vendorEditCtrl($scope, $modalInstance, vendorFactory , taxRuleFactory , displayName, $filter) {
	...
	vendorFactory.getVendorByName(displayName).success(function(data) {
		$scope.vendorForm = data.resultdata;
    ...
    $scope.ok = function () {
    	vendorFactory.updateVendor($scope.vendorForm).success(function(data) {
    		$scope.codeMasterVo = data.resultdata;
    		//console.log($scope.codeMasterVo);
    	}).error(function(data, status, headers, config) {
    		alert(data);
...
function vendorViewCtrl ($scope){
	
	$scope.vendors = vendorFactory.getVendors();	
	
}

function customerAddCtrl ($scope, $modalInstance, customerFactory, accountingCodeFactory , taxRuleFactory, $filter ) {
	
	$scope.customerForm = {};
	$scope.customerForm.required=false;	
	$scope.customerForm.showSuccessAlert = false;
	$scope.customerForm.showErrorAlert = false;
	$scope.customerForm.alerts1 = [
	                { type: 'danger', msg: 'aa' },
	                { type: 'success', msg: 'bb' },
	                { type: 'info', msg: 'cc' }]

	customerFactory.getInitData().success(function(data) {
		$scope.codeMasterVo = data.resultdata;
		//console.log($scope.codeMasterVo);
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	...
	taxRuleFactory.getAllRules().success(function(data) {
	 ...
	 $scope.changeCountry = function(){
			...
			var country = $filter('filter')($scope.countries, {
				code : $scope.customerForm.countryCode
			});	
    ...
    $scope.ok = function () {
    	customerFactory.addCustomer($scope.customerForm).success(function(data) {
    		$scope.codeMasterVo = data.resultdata;
    		//console.log($scope.codeMasterVo);
    	}).error(function(data, status, headers, config) {
    		alert( "failure message: " + JSON.stringify({data: data}));
...
function customerEditCtrl($scope, $modalInstance, customerFactory , displayName, $filter) {
	...
	customerFactory.getCustomerByName(displayName).success(function(data) {
		$scope.customerForm = data.resultdata;
	 ...
	 $scope.changeCountry = function(){
		...
		customerFactory.getInitData().success(function(data) {
		  ...
		  $scope.ok = function () {
		    	customerFactory.updateCustomer($scope.customerForm).success(function(data) {
		    		$scope.codeMasterVo = data.resultdata;
		    		//console.log($scope.codeMasterVo);
		    	}).error(function(data, status, headers, config) {
		    		alert(data);
...
function bankAddCtrl ($scope, $modalInstance, bankFactory, accountingCodeFactory , taxRuleFactory, $filter) {
	
	$scope.bankForm = {};
	$scope.bankForm.required=false;	
	$scope.bankForm.showSuccessAlert = false;
	$scope.bankForm.showErrorAlert = false;
	$scope.bankForm.alerts1 = [
	                { type: 'danger', msg: 'aa' },
	                { type: 'success', msg: 'bb' },
	                { type: 'info', msg: 'cc' }]

	bankFactory.getInitData().success(function(data) {
		$scope.codeMasterVo = data.resultdata;
		//console.log($scope.codeMasterVo);
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	...
	taxRuleFactory.getAllRules().success(function(data) {
	 ...
	 $scope.changeCountry = function(){
			...
			var country = $filter('filter')($scope.countries, {
				code : $scope.bankForm.countryCode
			});	
    ...
    $scope.ok = function () {
    	bankFactory.addBank($scope.bankForm).success(function(data) {
    		$scope.codeMasterVo = data.resultdata;
    		//console.log($scope.codeMasterVo);
    	}).error(function(data, status, headers, config) {
    		alert( "failure message: " + JSON.stringify({data: data}));
...
function employeeAddCtrl ($scope, $modalInstance, employeeFactory, accountingCodeFactory , taxRuleFactory) {
	
	$scope.employeeForm = {};
	$scope.employeeForm.required = false;	
	$scope.employeeForm.showSuccessAlert = false;
	$scope.employeeForm.showErrorAlert = false;
	$scope.employeeForm.alerts1 = [
	                { type: 'danger', msg: 'aa' },
	                { type: 'success', msg: 'bb' },
	                { type: 'info', msg: 'cc' }];

	employeeFactory.getInitData().success(function(data) {
		$scope.codeMasterVo = data.resultdata;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});
    ...
    $scope.ok = function () {
    	employeeFactory.addEmployee($scope.employeeForm).success(function(data) {
    		$scope.codeMasterVo = data.resultdata;
    	}).error(function(data, status, headers, config) {
    		alert( "failure message: " + JSON.stringify({data: data}));
    	});	
...
function fileUploadController($scope, FileUploader){
	...
	uploader.filters.push({
        ...
        fn: function(item /*{File|FileLikeObject}*/, options) {
        	...
        	var ul = item.size <= 10485760;
        	//console.log("size : " + ul);
        	var tl = '|jpg|JPG|png|PNG|jpeg|JPEG|bmp|BMP|gif|GIF|pdf|PDF'.indexOf(type) !== -1;
        	//console.log("typle : " + tl);
            return ul && tl;
        }
    });
...
function transactionCtrl($scope, $filter, $http, $q , transactionFactory , vendorFactory , accountingCodeFactory , taxRuleFactory) {
	$scope.txData = {};
	$scope.newTxCount = 0;
	
	$scope.saveMyData = function()
	{
		$scope.txMasterVo.newTxList = $scope.txList;
		$scope.txList ={};
		transactionFactory.saveTransaction($scope.txMasterVo).success(function(data) {
			$scope.newTxCount = 0;
			$scope.txMasterVo = data;
			$scope.txList = data.newTxList;
			$scope.editedTxList = data.editedTxList;
		}).error(function(data, status, headers, config) {
			alert( "failure message: " + JSON.stringify({data: data}));
		});
	...
	transactionFactory.getLandingData().success(function(data) {
		$scope.txMasterVo = data;
		$scope.txList = data.newTxList;
		$scope.editedTxList = data.editedTxList;
		for(var i=0; i < $scope.txList.length;i++){
			if($scope.txList[i].txDate != null)
			$scope.txList[i].txDate = new Date($scope.txList[i].txDate);
			$scope.txList[i].amount = parseFloat($scope.txList[i].amount).toFixed(2);
			$scope.txList[i].tax = parseFloat($scope.txList[i].tax).toFixed(2);
			$scope.txList[i].netAmount = parseFloat($scope.txList[i].netAmount).toFixed(2);
		}
		
		$('#codeVo_chosen_select_chosen').trigger('chosen:updated');
		$('#paymentEntity_chosen_select_chosen').trigger('chosen:updated');
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});
	 /* $scope.$watch('transaction.amount', function() {
	        $scope.transaction.amount = $scope.transaction.amount.toLowerCase().replace(/\s+/g,'');
	  });*/
	
	$scope.paymentMethodsVo = [
	        { "type" : "0", "name" : "0.Not Paid"},
	        { "type" : "1", "name" : "1.Cash"},
	        { "type" : "2", "name" : "2.Check"},
	        { "type" : "3", "name" : "3.Master Card"},
	        { "type" : "4", "name" : "4.Visa"},
	        { "type" : "5", "name" : "5.E-Check"}
	];
	...
	$scope.deleteTransaction = function(transaction,index) {
		var filtered = $filter('filter')($scope.txList, {
	...
	$scope.addTransaction = function() {
		$scope.newTxCount = $scope.newTxCount + 1;
		$scope.txList.push({
			transactionId : 'new' + $scope.newTxCount,
			paymentMethod : '0',
			isNew : 'true'
		});
	};
	
	$scope.duplicateTransaction = function(transaction, index) {
		$scope.newTxCount = $scope.newTxCount + 1;
		var baseTx = transaction;
		var newTx = {
				transactionId : 'new' + $scope.newTxCount ,
				acccountSubCatId : baseTx.acccountSubCatId,
				accountCatId : baseTx.accountCatId,
				accountId : baseTx.accountId,
				accountTypeId : baseTx.accountTypeId,
				accountUsername : baseTx.accountUsername,
				accountantId : baseTx.accountantId,
				amount : baseTx.amount,
				balanceDue : baseTx.balanceDue,
				codeVo : baseTx.codeVo,
				customerVo : baseTx.customerVo,
				description : baseTx.description,
				discountPercent : baseTx.discountPercent,
				entityId : baseTx.entityId,
				fileId : baseTx.fileId,
				filename : baseTx.filename,
				itemDesc : baseTx.itemDesc,
				lastModifiedDate: '',
				name : baseTx.name,
				paymentAmount : baseTx.paymentAmount,
				paymentDate : baseTx.paymentDate,
				paymentEntity : baseTx.paymentEntity,
				paymentEntityId : baseTx.paymentEntityId,
				paymentMethod : baseTx.paymentMethod,
				quantity : baseTx.quantity,
				rate : baseTx.rate,
				referenceId : baseTx.referenceId,
				remarks : baseTx.remarks,
				status : baseTx.status,
				tax : baseTx.tax,
				taxType : baseTx.taxType,
				txCategory : baseTx.txCategory,
				txCategoryId : baseTx.txCategoryId,
				txClass : baseTx.txClass,
				txClassId : baseTx.txClassId,
				txDate : baseTx.txDate,
				txMode : baseTx.txMode,
				txSet : baseTx.txSet,
				txType : baseTx.txType,
				vendorVo : baseTx.vendorVo,
				netAmount : baseTx.netAmount,
				isNew : 'true'
		}
		
		$scope.txList.splice(index + 1 , 0 , newTx);
	};

	// cancel all changes
	$scope.cancel = function() {
		for (var i = $scope.txList.length; i--;) {
			var transaction = $scope.txList[i];
			// undelete
	...
	//Vendor suggestion
	  $scope.getLocation = function(val) {
	    return $http.post('/vendor/suggest-name', {
	      params: {
	    	search: val,
	        sensor: false
	      }
	...
	//Get vendor details
	$scope.onSelect = function ($item, transaction) {
	    $scope.$item = $item;
	    vendorFactory.getVendorByName($item).success(function(data) {
			$scope.updateTransaction(transaction,data.resultdata);
		}).error(function(data, status, headers, config) {
			alert( "failure message: " + JSON.stringify({data: data}));
		});	
    };
	...
	//TaxRule suggestion
	$scope.suggestTaxRule = function(val) {
		    return $http.post('/taxrule/suggest-rule', {
		      params: {
		    	search: val,
		        sensor: false
		      }
	...
	//Get Taxrule details
	$scope.onTaxRuleSuggestSelect = function ($item, transaction) {
	    $scope.$item = $item;
		transaction.taxType = $item;
    };
    
    $scope.onAmountChange = function(transaction)
    {
    	var newAmt = 0;
    	...
    	if(  transaction.taxType !== "undefined" &&  transaction.rate !== "undefined")
    	{
    		transaction.netAmount = ( transaction.amount * 100 ) / (100 + transaction.rate * 1);
    		transaction.netAmount = parseFloat(transaction.netAmount).toFixed(2);
			transaction.tax = transaction.amount - transaction.netAmount;
			transaction.tax = parseFloat(transaction.tax).toFixed(2);
    	}
    		
    }
    
    $scope.onTaxTypeChange = function(transaction, taxType)
    {
    	
    	for (var j = $scope.taxRuleList.length; j--;) {
			if(taxType == $scope.taxRuleList[j].displayName)
			{
				transaction.taxType = $scope.taxRuleList[j].displayName;
				transaction.rate = $scope.taxRuleList[j].percentage;
				break;
			}
		}
    ...
    $scope.updateTransaction = function(transaction, vendorVo ) {
    	transaction.vendorVo = vendorVo;
    	transaction.codeVo = vendorVo.codeVo;
		for (var j = $scope.taxRuleList.length; j--;) {
			if(vendorVo.stateCode == $scope.taxRuleList[j].stateCode)
			{
				transaction.taxType = $scope.taxRuleList[j].displayName;
				transaction.rate = $scope.taxRuleList[j].percentage;
...
function transactionTwoCtrl($scope, $filter, $http, $q , transactionTwoFactory , customerFactory , accountingCodeFactory , taxRuleFactory) {
	...
	$scope.saveMyData = function()
	{
		...
		transactionTwoFactory.saveTransaction($scope.txMasterVo).success(function(data) {
			$scope.txMasterVo = data;
		}).error(function(data, status, headers, config) {
			alert( "failure message: " + JSON.stringify({data: data}));
		});
		$scope.txList = new Object();
	}
	...
	transactionTwoFactory.getLandingData().success(function(data) {
		$scope.txMasterVo = data;
		$scope.txList = data.newTxList;
	...
	//Vendor suggestion
	  $scope.getLocation = function(val) {
	    return $http.post('/customer/suggest-name', {
	      params: {
	    	search: val,
	        sensor: false
	      }
	...
	$scope.onSelect = function ($item, transaction) {
	    $scope.$item = $item;
	    customerFactory.getCustomerByName($item).success(function(data) {
			$scope.updateTransaction(transaction,data.resultdata);
		}).error(function(data, status, headers, config) {
			alert( "failure message: " + JSON.stringify({data: data}));
		});	
    ...
    $scope.onTaxTypeChange = function(transaction, taxType)
    {
	 ...
	 $scope.updateTransaction = function(transaction, customerVo ) {
		 	transaction.customerVo = customerVo;
		 	transaction.codeVo = customerVo.codeVo;
		 	
		 	for (var j = $scope.taxRuleList.length; j--;) {
				if(customerVo.stateCode == $scope.taxRuleList[j].stateCode)
				{
					transaction.taxType = $scope.taxRuleList[j].displayName;
					transaction.rate = $scope.taxRuleList[j].percentage;
...
function MainCtrl($scope,mainFactory,$rootScope) {
	...
	$rootScope.countries = this.countries;
	$rootScope.states = this.countries[0].states;
...
function modalDemoCtrl($scope, $modal) {
    ...
    $scope.open = function () {
        ...
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/modal_example.html',
            controller: ModalInstanceCtrl,
        });
    };
    ...
    $scope.open1 = function () {
        var modalInstance = $modal.open({
            templateUrl: 'assets/dashboard/views/modal_example1.html',
            controller: ModalInstanceCtrl
        });
    };
...
function stepOneHandler($scope,$state,oneFactory,$rootScope,$filter) {
	// All data will be store in this object
	$scope.formData = {};
    ...
    $scope.changeCountry = function(){
		...
		$scope.states = country[0].states;
		//$scope.formData.province = $scope.states[0];
		
	};
	
	oneFactory.getFormOneData().success(function(data) {
		$scope.formData = data;
		...
		{
			$scope.formData.country = $scope.countries[0].code;
		}
		...
		{
			$scope.formData.province = $scope.states[0].code;	
		}
	...
	$scope.processForm = function() {
		...
		var res = oneFactory.submitFormOne($scope.formData);
		res.success(function(data, status, headers, config) {
			$state.go('profile.landing.step_two');
		});
...
function stepTwoHandler($scope,$state,twoFactory,$rootScope,$filter) {
	// All data will be store in this object
	$scope.formData = {};
	...
	$scope.changeLegalCountry = function(){
		var country = $filter('filter')($scope.countries, {
			code : $scope.formData.legalCountry
		});	
		$scope.states = country[0].states;
		//$scope.formData.province = $scope.states[0];
	};
	
    twoFactory.getFormOneData().success(function(data) {
		$scope.formData = data;
		
		if(data.legalCountry != 'undefined' &&  data.legalCountry != '')
		{
			$scope.formData.legalCountry = data.legalCountry;
		}
		else
		{
			$scope.formData.legalCountry = $scope.countries[0].code;
		}
		
		if(data.legalProvince != 'undefined' &&  data.legalProvince != '')
		{
			$scope.formData.legalProvince = data.legalProvince;
		}
		else
		{
			$scope.formData.legalProvince = $scope.states[0].code;	
		}
		
		$scope.changeLegalCountry();
	...
	$scope.processForm = function() {
		var res = twoFactory.submitFormTwo($scope.formData);
		res.success(function(data, status, headers, config) {
			
			
			$rootScope.profileNotification = true;
			//$state.go('profile.landing.step_three');
			$state.go('dashboard.0');
		});
		res.error(function(data, status, headers, config) {
			alert("Some error while processing your request");
		});
...
function stepThreeHandler($scope,$state,threeFactory) {
	// All data will be store in this object
	$scope.formData = {};
	threeFactory.getFormOneData().success(function(data) {
		$scope.formData.terms = data.terms;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});	
	$scope.processForm = function() {
		var res = threeFactory.submitFormThree($scope.formData);
		res.success(function(data, status, headers, config) {
			$state.go('dashboard.0');
		});
		res.error(function(data, status, headers, config) {
...
function addUserController($scope, $state, addUserFactory) {
	// All data will be store in this object
	$scope.formData = {};
	$scope.formData.required=false;	
	$scope.formData.showSuccessAlert = false;
	$scope.formData.showErrorAlert = false;
	$scope.formData.alerts1 = [
	                { type: 'danger', msg: 'aa' },
	                { type: 'success', msg: 'bb' },
	                { type: 'info', msg: 'cc' },
	            ];
	addUserFactory.getInitialData().success(function(data) {
		$scope.packageList = data.packageName;
		$scope.roleList = data.role;
		$scope.formData.packageName = $scope.packageList[0];
	...
	$scope.processForm = function() {
		var res = addUserFactory.addUser($scope.formData);
		res.success(function(data, status, headers, config) {
			$scope.formData.showSuccessAlert = true;
			$state.go('dashboard.0');
		});
		res.error(function(data, status, headers, config) {
			$scope.formData.showErrorAlert = true;
		});

	};
}

function deActivateController($scope,$http, $state,deActivateUserFactory){
	$scope.myModel = null;
	...
	$scope.getNames = function(val) {
	    return $http.post('/manageuser/suggestUser', {
	      ...
	      return response.data.resultdata.splice(0,10);
	    });
	  };
	$scope.processData = function(){
		$scope.formData.searchUser = true;
		var res = deActivateUserFactory.searchUser($scope.formData);
		res.success(function(data, status, headers, config) {
			$scope.showFields=true;
			$scope.phNoOfUser = data.phoneNumber;
			$scope.mailIdOfUser = data.email;
		...
		$scope.deactivateAccount = function (){
			var res = deActivateUserFactory.deactivateAccount($scope.formData);
			res.success(function(data, status, headers, config) {
...
function reActivateController($scope,$http, $state,reActivateUserFactory){
	$scope.myModel = null;
    $scope.myModelId = null;
    $scope.isDisabled = true;
	$scope.formData = {};
	...
	$scope.processData = function(){
		$scope.formData.searchUser = true;
		var res = reActivateUserFactory.searchUser($scope.formData);
		res.success(function(data, status, headers, config) {
			$scope.showFields=true;
			$scope.phNoOfUser = data.phoneNumber;
			$scope.mailIdOfUser = data.email;
		...
		$scope.reactivateAccount = function (){
			var res = reActivateUserFactory.reactivateAccount($scope.formData);
			res.success(function(data, status, headers, config) {
				$scope.showSuccessAlert = true;
				$scope.showErrorAlert = false;
			});
...
function passwordResetController($scope,$http,	 $state,resetPassword){
	$scope.myModel = null;
    $scope.myModelId = null;
    $scope.isDisabled = true;
	$scope.formData = {};
	...
	$scope.processData = function(){
		$scope.formData.searchUser = true;
		var res = resetPassword.searchUser($scope.formData);
		res.success(function(data, status, headers, config) {
			$scope.showFields=true;
			$scope.phNoOfUser = data.phoneNumber;
			$scope.mailIdOfUser = data.email;
	...
	$scope.resetPassword = function(){
		var res = resetPassword.reset($scope.formData);
		res.success(function(data, status, headers, config) {
			$scope.showErrorAlert = false;
			$scope.showSuccessAlert = true;
		});
...
function dashboardController($scope, $rootScope, notify){
	
	if($rootScope.profileNotification == true){
		notifyUser();
		
	}
	
	 function notifyUser() {
		 notify({ message: 'Your profile has been saved', classes: 'alert-info'});
    };
...
function notifyCtrl($scope, notify) {
    ...
    $scope.yoursiteTemplate = 'assets/dashboard/views/common/notify.html';
    $scope.yoursiteDemo1 = function(){
        notify({ message: 'Info - This is a yoursite info notification', classes: 'alert-info', templateUrl: $scope.yoursiteTemplate});
    }
    $scope.yoursiteDemo2 = function(){
        notify({ message: 'Success - This is a yoursite success notification', classes: 'alert-success', templateUrl: $scope.yoursiteTemplate});
    }
    $scope.yoursiteDemo3 = function(){
        notify({ message: 'Warning - This is a yoursite warning notification', classes: 'alert-warning', templateUrl: $scope.yoursiteTemplate});
    }
    $scope.yoursiteDemo4 = function(){
        notify({ message: 'Danger - This is a yoursite danger notification', classes: 'alert-danger', templateUrl: $scope.yoursiteTemplate});
    }
}
...
function balanceSheetController($scope,reportingFactory){
	$scope.sheetInfo= {};
	reportingFactory.getBalanceSheet().success(function(data) {
		$scope.sheetInfo = data.data;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});	
}

function incomeStatementController($scope,reportingFactory){
	$scope.sheetInfo= {};
	reportingFactory.getIncomeStatement().success(function(data) {
		$scope.sheetInfo = data.data;
	}).error(function(data, status, headers, config) {
		alert( "failure message: " + JSON.stringify({data: data}));
	});	
...
function signupController($scope) {
	$scope.formData = {};
    $scope.formData.alreadyExist = false;
  }
...
angular
    ...
    .controller('stepOneHandler', stepOneHandler)
    .controller('stepTwoHandler', stepTwoHandler)
    .controller('stepThreeHandler', stepThreeHandler)
	...
	.controller('reportFileCtrl', reportFileCtrl)
	.controller('taxRuleCtrl', taxRuleCtrl)
	.controller('signupController', signupController)
	.controller('vendorViewCtrl', vendorViewCtrl)
